    ...
    public func goToNextState() {
        ...
        case .withHuman:
            ...
            if let playerInputState = currentState as? PlayerInputState {
             ...
             self.currentState = PlayerInputState(player: player, markViewPrototype: player.markViewPrototype, gameVC: self, gameboard: gameboard, gameboardView: rootView.gameboardView)
            }
        case .withAI:
            guard !isEndGame() else {return}
          ...
          if let playerInputState = currentState as? PlayerInputState {
               let player = playerInputState.player.next
               self.currentState = AIInputState(isCompleted: false, markViewPrototype: player.markViewPrototype, player: player, gameVC: self, gameboard: gameboard, gameboardView: rootView.gameboardView)
           }
        ...
        case .blindGame:
            if let playerInputState = currentState as? PlayerInputBGState {
                ...
                if playerInputState.count < 5 {
                    self.currentState = PlayerInputBGState(player: player, markViewPrototype: prototype, gameVC: self, gameboard: gameboard, count: playerInputState.count, gameboardView: rootView.gameboardView)
                    ...
                    if playerInputState.player == .second {
                        self.currentState = BlindGameState(player: player, markViewPrototype: prototype, gameVC: self, gameboard: gameboard, gameboardView: rootView.gameboardView)
                    }
                    else {
                        self.currentState = PlayerInputBGState(player: player, markViewPrototype: prototype, gameVC: self, gameboard: gameboard, count: 0, gameboardView: rootView.gameboardView)
                    }
                }
                ...
                let player = playerInputState.player.next
                self.currentState = BlindGameState(player: player, markViewPrototype: player.markViewPrototype, gameVC: self, gameboard: gameboard, gameboardView: rootView.gameboardView)
            }
        }
    }
